...
const photoInfo = document.querySelector("img");
...
function updateInfos() {
    ...
    setTimeout(() => progress.classList.add("animate"), 20);
    const { name, position, photo, text } = users[idx];

    textInfo.innerText = text;
    nameInfo.innerText = name;
    positionInfo.innerText = position;
    photoInfo.src = photo;

    idx == users.length - 1 ? idx = 0 : idx++;
